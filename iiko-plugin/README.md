...
4. В iikoOffice/backoffice заведи тип оплаты, связанный с плагином — см.
   [Настройка типа оплаты в backoffice](#настройка-типа-оплаты-в-backoffice)
   ниже (поля + счета).
...
### Настройка типа оплаты в backoffice

iikoOffice → **Розничные продажи → Типы оплат → Добавить**. Плагин при старте
регистрирует платёжную систему с ключом `revisitr` («Revisitr Бонусы») — она
появляется в списке **«Безналичный тип»** после первого запуска iikoFront с
плагином.

| Поле | Значение | Почему |
|------|----------|--------|
| **Наименование** | `Revisitr Бонусы` | как гость увидит на экране оплаты |
| **Тип** | `Внешний тип оплаты` | категория для плагинных платёжек |
| **Безналичный тип** | `Revisitr Бонусы` (ключ `revisitr`) | связывает тип оплаты именно с нашим плагином — **не** выбирать чужой (iikoCard5, YandexEda и т.п.) |
| **Является фискальным** | ❌ выкл | бонусы — не фискальные деньги; нефискальный внешний тип попадает в раздел «Безналичный расчёт» |
| **Можно комбинировать с другими типами оплаты** | ✅ вкл | часть чека — бонусами, часть — картой/наличными |

⚠️ Если в списке «Безналичный тип» нет пункта `Revisitr Бонусы` — значит плагин
не загрузился (проверь `plugin-<folder>.log`). Наличие этого пункта = плагин
зарегистрировался.

#### Счета: Счёт выручки / Счёт поступления денег

Оплата бонусами — это **не реальные деньги**. Поэтому «Счёт поступления денег»
НЕ должен быть счётом реального эквайринга/налички — иначе завышаются касса и
денежный поток.

- **Счёт выручки** — оставить `Торговая выручка` (товар всё равно продан, выручку
  признаём). Чтобы выделить канал, можно создать подсчёт `Торговая выручка —
  бонусы Revisitr` (План счетов → ПКМ по «Торговая выручка» → «Создать подсчёт»).
- **Счёт поступления денег** — **НЕ** `Денежные средства, эквайринг`. Завести
  отдельный счёт/подсчёт, например `Оплата бонусами Revisitr`, под статьёй
  маркетинговых расходов (или бонусным «виртуальным» счётом). Погашенная бонусами
  сумма осядет там как стоимость программы лояльности, а не как приход денег.

Точный счёт зависит от Плана счетов конкретного заведения — **согласуйте с
бухгалтером**. Аналогично устроен штатный бонусный тип оплаты iikoCard5
(«Безналичный расчёт» + системный безналичный тип, нефискальный).

Источники (сверено):
[Внешний тип оплаты — iikoOffice](https://ru.iiko.help/smart/project-iikooffice/topic-103),
[Руководство iikoCard5 (PDF)](https://rapid.iiko.ru/userguide/iikoCard5/UG_iikoCard.pdf),
[Учёт денежных средств в iiko (PDF)](https://rapid.iiko.ru/userguide/Ispolzovanie_iiko/Uchet_denezhnyx_sredstv.pdf).
