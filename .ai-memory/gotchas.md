...
- **Wallet**: ~95% done. Apple pkpass generation, PKCS#7 signing, web service (incl. device registration + APNs push updates via `.p8`), and `RefreshPassBalance` are implemented and verified against the real production `.p12` (signature chains leaf→WWDR G4→Apple Root, `openssl smime -verify` passes). See `docs/wallet/README.md`. **Apple pkpass gotchas**: (1) the Apple WWDR intermediate is REQUIRED in the signature but `.p12` bundles usually ship only the leaf — so WWDR G4 is embedded in `internal/usecase/wallet/wwdr.go` and auto-appended to the chain. It expires 2030-12-10; if Apple starts issuing Pass Type ID certs under a newer WWDR generation, embed that one too. (2) `.p12` with a SHA-256 MAC (modern openssl export) is NOT decodable by `golang.org/x/crypto/pkcs12` — the code uses `software.sslmate.com/src/go-pkcs12` (`DecodeChain`) instead. (3) `webServiceURL` in pass.json MUST be HTTPS — with an `http://` (e.g. localhost) URL iOS silently refuses to recognize the file as a pass (offers "save to iCloud Drive" instead of the Add-to-Wallet sheet). On-device add verified 2026-07-23 with the production cert.
