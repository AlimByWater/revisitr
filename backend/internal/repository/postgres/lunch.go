...
func (r *Lunch) CreateOrder(ctx context.Context, o *entity.LunchOrder) error {
	tx, err := r.pg.DB().BeginTxx(ctx, nil)
	if err != nil {
		return fmt.Errorf("lunch.CreateOrder begin: %w", err)
	}
	defer tx.Rollback()

	if err := tx.QueryRowContext(ctx, `
		INSERT INTO lunch_orders (bot_id, bot_client_id, format_id, format_name, table_num, total_price, status)
		VALUES ($1, $2, $3, $4, $5, $6, $7)
		RETURNING id, created_at`,
		o.BotID, o.BotClientID, o.FormatID, o.FormatName, o.TableNum, o.TotalPrice, o.Status,
	).Scan(&o.ID, &o.CreatedAt); err != nil {
		return fmt.Errorf("lunch.CreateOrder insert: %w", err)
	}

	for i := range o.Items {
		item := &o.Items[i]
		item.LunchOrderID = o.ID
		if err := tx.QueryRowContext(ctx, `
			INSERT INTO lunch_order_items (lunch_order_id, course_id, course_title, menu_item_id, item_name, price, surcharge)
			VALUES ($1, $2, $3, $4, $5, $6, $7)
			RETURNING id`,
			item.LunchOrderID, item.CourseID, item.CourseTitle, item.MenuItemID,
			item.ItemName, item.Price, item.Surcharge,
		).Scan(&item.ID); err != nil {
			return fmt.Errorf("lunch.CreateOrder item: %w", err)
		}
	}

	if err := tx.Commit(); err != nil {
		return fmt.Errorf("lunch.CreateOrder commit: %w", err)
	}
	return nil
}

func (r *Lunch) ListOrders(ctx context.Context, botID int, status string) ([]entity.LunchOrder, error) {
	query := "SELECT * FROM lunch_orders WHERE bot_id = $1"
	args := []any{botID}
	if status != "" {
		query += " AND status = $2"
	...
	var orders []entity.LunchOrder
	if err := r.pg.DB().SelectContext(ctx, &orders, query, args...); err != nil {
		return nil, fmt.Errorf("lunch.ListOrders: %w", err)
	}
	for i := range orders {
		if err := r.pg.DB().SelectContext(ctx, &orders[i].Items,
			"SELECT * FROM lunch_order_items WHERE lunch_order_id = $1 ORDER BY id",
			orders[i].ID); err != nil {
			return nil, fmt.Errorf("lunch.ListOrders items: %w", err)
		}
	}
	return orders, nil
}

func (r *Lunch) UpdateOrderStatus(ctx context.Context, orderID int, status string) error {
	_, err := r.pg.DB().ExecContext(ctx,
		"UPDATE lunch_orders SET status = $2 WHERE id = $1", orderID, status)
	if err != nil {
		return fmt.Errorf("lunch.UpdateOrderStatus: %w", err)
	}
	return nil
}

func (r *Lunch) GetOrderOrgID(ctx context.Context, orderID int) (int, error) {
	var orgID int
	err := r.pg.DB().GetContext(ctx, &orgID, `
		SELECT b.org_id FROM lunch_orders lo
		JOIN bots b ON b.id = lo.bot_id
		WHERE lo.id = $1`, orderID)
	if err != nil {
		if err == sql.ErrNoRows {
			return 0, nil
		}
		return 0, fmt.Errorf("lunch.GetOrderOrgID: %w", err)
	}
	return orgID, nil
}
